...
if(_PAIMON_PROTOBUF_LIBRARY_TARGET AND NOT PROTOBUF_COMPILER)
    find_program(PROTOBUF_COMPILER NAMES protoc ${_PAIMON_PROTOBUF_FIND_ARGS}
                 PATH_SUFFIXES bin)
endif()
...
if(_PAIMON_PROTOBUF_LIBRARY_TARGET)
    ...
    if(ProtobufAlt_FOUND)
        get_target_property(PROTOBUF_INCLUDE_DIR
        ...
        if(NOT TARGET libprotobuf)
            ...
            if(PROTOBUF_INCLUDE_DIR)
                set_target_properties(
                    libprotobuf PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
        ...
        if(_PAIMON_PROTOC_LIBRARY_TARGET AND NOT TARGET libprotoc)
            ...
            if(PROTOBUF_INCLUDE_DIR)
                set_target_properties(
                    libprotoc PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                         "${PROTOBUF_INCLUDE_DIR}")
            endif()
            target_link_libraries(libprotoc
                                  INTERFACE ${_PAIMON_PROTOC_LIBRARY_TARGET})
        endif()
    ...
    find_path(PROTOBUF_INCLUDE_DIR NAMES google/protobuf/message.h
              ${_PAIMON_PROTOBUF_FIND_ARGS} HINTS ${PC_Protobuf_INCLUDE_DIRS}
              PATH_SUFFIXES include)
    find_library(PROTOBUF_LIBRARY NAMES protobuf
                 ${_PAIMON_PROTOBUF_FIND_ARGS}
                 HINTS ${PC_Protobuf_LIBRARY_DIRS} PATH_SUFFIXES lib lib64)
    find_library(PROTOC_LIBRARY NAMES protoc ${_PAIMON_PROTOBUF_FIND_ARGS}
                 HINTS ${PC_Protobuf_LIBRARY_DIRS} PATH_SUFFIXES lib lib64)
    find_program(PROTOBUF_COMPILER NAMES protoc ${_PAIMON_PROTOBUF_FIND_ARGS}
                 PATH_SUFFIXES bin)
    ...
    if(ProtobufAlt_FOUND)
        ...
        if(PROTOC_LIBRARY AND NOT TARGET libprotoc)
            ...
            set_target_properties(
                libprotoc PROPERTIES IMPORTED_LOCATION "${PROTOC_LIBRARY}"
                                     INTERFACE_INCLUDE_DIRECTORIES
                                     "${PROTOBUF_INCLUDE_DIR}")
