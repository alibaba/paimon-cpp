...
TEST_P(ScanAndReadInteTest, TestAvroWithAppendSnapshot1) {
    auto [file_format, enable_prefetch] = GetParam();
    if (file_format != "avro") {
        return;
    }
...
TEST_P(ScanAndReadInteTest, TestAvroWithAppendSnapshot2) {
    auto [file_format, enable_prefetch] = GetParam();
    if (file_format != "avro") {
        return;
    }
...
TEST_P(ScanAndReadInteTest, TestAvroWithPkSnapshot1) {
    auto [file_format, enable_prefetch] = GetParam();
    if (file_format != "avro") {
        return;
    }
    std::string table_path = GetDataDir() + "/avro/pk_with_multiple_type.db/pk_with_multiple_type";
    ...
    auto expected = std::make_shared<arrow::ChunkedArray>(
        arrow::ipc::internal::json::ArrayFromJSON(struct_(fields), R"([
...
TEST_P(ScanAndReadInteTest, TestAvroWithPkSnapshot2) {
    auto [file_format, enable_prefetch] = GetParam();
    if (file_format != "avro") {
        return;
    }
