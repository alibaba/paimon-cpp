...
// Performance test: lookup compact with DV on a real-world table.
// Copies the mainse_pk table (310MB level-5 + 156MB level-0, 1177 fields, PK=nid) to a temp dir,
// modifies schema to enable DV + lookup compact, then triggers non-full compact.
// Verifies:
//   1. DV index files are produced (level-5 file gets DV marks via lookup)
//   2. Level-5 file is NOT in CompactBefore (not part of CompactUnit, not rewritten)
//   3. Reports compact wall-clock time
TEST_F(PkCompactionInteTest, DISABLED_LookupCompactWithDV) {
    // Step 1: Copy the original table to a temp directory.
    std::string src_table_path =
        "/home/lxy264173/open_source/test/fork3/lookup_test/parquet/mainse_pk";
    std::string table_path = PathUtil::JoinPath(dir_->Str(), "mainse_pk");
    ASSERT_TRUE(TestUtil::CopyDirectory(src_table_path, table_path));

    // Step 2: Modify schema-0 to add DV + lookup compact options.
    // Read the original schema, parse JSON, add options, write back.
    {
        auto fs = std::make_shared<LocalFileSystem>();
        std::string schema_path = table_path + "/schema/schema-0";
        std::string schema_content;
        ASSERT_OK(fs->ReadFile(schema_path, &schema_content));

        // Parse and modify the JSON schema: add DV and lookup compact options,
        // remove write-only so compact can run.
        // We do simple string replacement on the options block.
        // Original options block:
        //   "bucket": "1",
        ...
        // And remove "write-only": "true"

        // Remove write-only option
        {
            auto pos = schema_content.find("\"write-only\" : \"true\",");
            if (pos != std::string::npos) {
                // Remove the line including trailing newline
                auto end = schema_content.find('\n', pos);
                if (end != std::string::npos) {
                    schema_content.erase(pos, end - pos + 1);
                }
            }
        }

        // Add deletion-vectors.enabled before the closing brace of options
        // Find the last option line before the closing "}" of options
        {
            // Insert new options right after "bucket" : "1" line
            auto pos = schema_content.find("\"bucket\" : \"1\"");
            if (pos != std::string::npos) {
                auto end_of_line = schema_content.find('\n', pos);
                if (end_of_line != std::string::npos) {
                    std::string new_options =
                        "\n    \"deletion-vectors.enabled\" : \"true\","
                        "\n    \"merge-engine\" : \"aggregation\","
                        "\n    \"fields.default-aggregate-function\" : \"max\",";
                    schema_content.insert(end_of_line, new_options);
                }
            }
        }
        ...
        ASSERT_OK(fs->WriteFile(schema_path, schema_content, /*overwrite=*/false));

        // Verify the schema was modified correctly
        std::string verify_content;
        ASSERT_OK(fs->ReadFile(schema_path, &verify_content));
        ASSERT_TRUE(verify_content.find("deletion-vectors.enabled") != std::string::npos)
            << "Schema should contain deletion-vectors.enabled";
        ASSERT_TRUE(verify_content.find("write-only") == std::string::npos)
            << "Schema should not contain write-only";
    }

    // Step 3: Run non-full compact and measure time.
    auto start_time = std::chrono::steady_clock::now();

    ASSERT_OK_AND_ASSIGN(
        auto compact_msgs,
        CompactAndCommit(table_path, {}, 0, /*full_compaction=*/false, /*commit_identifier=*/10));

    auto end_time = std::chrono::steady_clock::now();
    auto elapsed_ms =
        std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time).count();
    std::cout << "=== Lookup Compact with DV: " << elapsed_ms << " ms ===" << std::endl;

    // Step 4: Verify DV index files were produced.
    ASSERT_TRUE(HasDeletionVectorIndexFiles(compact_msgs))
        << "Compact should produce DV index files (lookup against level-5 file)";

    // Step 5: Verify compact result details via CommitMessage.
    for (const auto& msg : compact_msgs) {
        auto impl = dynamic_cast<CommitMessageImpl*>(msg.get());
        if (!impl) continue;
        ...
        std::cout << "  CompactBefore files: " << compact_before.size() << std::endl;
        for (const auto& meta : compact_before) {
            std::cout << "    " << meta->file_name << " level=" << meta->level
                      << " size=" << meta->file_size << std::endl;
        }
        std::cout << "  CompactAfter files: " << compact_after.size() << std::endl;
        for (const auto& meta : compact_after) {
            std::cout << "    " << meta->file_name << " level=" << meta->level
                      << " size=" << meta->file_size << std::endl;
        }

        // The level-5 file name should NOT appear in CompactBefore
        // (it was not part of the CompactUnit, only level-0 was selected).
        for (const auto& meta : compact_before) {
            ASSERT_NE(meta->file_name, "data-c3939beb-81cd-4d81-a413-41a4ba18ce24-0.parquet")
                << "Level-5 file should NOT be in CompactBefore (should not be rewritten)";
        }
    }

    std::cout << "=== Test passed: level-0 rewritten, level-5 only got DV ===" << std::endl;
}
