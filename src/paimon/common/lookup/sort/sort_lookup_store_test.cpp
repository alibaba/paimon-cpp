...
TEST(SortLookupStoreTest, TestSimple) {
    ...
    std::shared_ptr<arrow::Array> key_array = arrow::ipc::internal::json::ArrayFromJSON(key_type,
                                                                                        R"([
        ["Alex"],
        ["Alice"],
        ["Bob"],
    ...
    ASSERT_OK_AND_ASSIGN(auto key_serializer,
                         RowCompactedSerializer::Create(arrow::schema(key_type->fields()), pool));
    ASSERT_OK_AND_ASSIGN(auto value_serializer,
                         RowCompactedSerializer::Create(arrow::schema(value_type->fields()), pool));
    // write data
