...
TEST(ReaderUtilsTest, TestRemoveFieldFromStructArrayFieldNotFound) {
    auto struct_type =
    ...
    ASSERT_OK_AND_ASSIGN(auto result,
                         ReaderUtils::RemoveFieldFromStructArray(src_struct_array, "missing"));

    ASSERT_TRUE(result->Equals(src_struct_array));
    ASSERT_EQ(result->type()->num_fields(), 2);
}

TEST(ReaderUtilsTest, TestRemoveFieldFromStructArraySuccess) {
    auto struct_type =
        arrow::struct_({arrow::field("a", arrow::int32()), arrow::field("b", arrow::utf8()),
    ...
    ASSERT_OK_AND_ASSIGN(auto result,
                         ReaderUtils::RemoveFieldFromStructArray(src_struct_array, "b"));
