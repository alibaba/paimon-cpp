...
int64_t BinaryInMemorySortBuffer::Size() const {
    int64_t total = 0;
    for (const auto& batch : buffered_batches_) {
        total += batch.struct_array->length();
    }
    return total;
}

Status BinaryInMemorySortBuffer::Clear() {
...
BinaryInMemorySortBuffer::SortedIterator() {
    ...
    for (auto& buffered_batch : buffered_batches_) {
        auto in_memory_reader = std::make_unique<KeyValueInMemoryRecordReader>(
            buffered_batch.first_sequence_number, std::move(buffered_batch.struct_array),
            std::move(buffered_batch.row_kinds), trimmed_primary_keys_,
            user_defined_sequence_fields_, sequence_fields_ascending_, key_comparator_, pool_);
