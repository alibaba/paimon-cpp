...
Status CreateMemoryLimitExceededStatus(uint64_t max_memory) {
    return Status::Invalid(
        fmt::format("single write batch exceeds global write-buffer-size limit {} bytes; "
                    "please increase write-buffer-size or reduce batch size",
                    max_memory));
}
...
Status WriterMemoryManager::ShrinkToLimit() {
    ...
    while (true) {
        ...
        if (picked.memory == 0) {
            return CreateMemoryLimitExceededStatus(max_memory_);
        }
