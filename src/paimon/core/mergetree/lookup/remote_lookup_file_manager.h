...
class RemoteLookupFileManager {
 public:
    RemoteLookupFileManager(int32_t level_threshold,
                            ...
                            const std::shared_ptr<MemoryPool>& pool, LookupLevels<T>* lookup_levels)
    ...
    Result<std::shared_ptr<DataFileMeta>> GenRemoteLookupFile(
        ...
        PAIMON_RETURN_NOT_OK(
            CopyFromInputToOutput(std::move(input_stream), std::move(output_stream)));
    ...
    /// Try to download a remote lookup file to local.
    bool TryToDownload(const std::shared_ptr<DataFileMeta>& data_file,
                       const std::string& remote_sst_file, const std::string& local_file) {
 ...
 private:
    ...
    Status CopyRemoteToLocal(const std::string& remote_path, const std::string& local_path) {
        PAIMON_ASSIGN_OR_RAISE(std::unique_ptr<InputStream> input_stream,
