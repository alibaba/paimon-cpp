...
Status MergeTreeWriter::DoClose() {
    ...
    PAIMON_RETURN_NOT_OK(write_buffer_->Clear());
    new_files_.clear();
...
Status MergeTreeWriter::FlushMemory() {
    PAIMON_ASSIGN_OR_RAISE(bool spilled, write_buffer_->FlushMemory());
    if (!spilled) {
...
Status MergeTreeWriter::Write(std::unique_ptr<RecordBatch>&& moved_batch) {
    PAIMON_ASSIGN_OR_RAISE(bool buffered, write_buffer_->Write(std::move(moved_batch)));
    if (!buffered) {
