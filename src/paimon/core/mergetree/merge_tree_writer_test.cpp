...
class MergeTreeWriterTest : public ::testing::TestWithParam<bool> {
 public:
    ...
    size_t CountFiles(const std::string& dir_path) const {
        auto exists = file_system_->Exists(dir_path);
        if (!exists.ok() || !exists.value()) {
            return 0;
        }
        std::vector<std::unique_ptr<FileStatus>> file_statuses;
        EXPECT_OK(file_system_->ListFileStatus(dir_path, &file_statuses));
        return file_statuses.size();
    }
...
TEST_P(MergeTreeWriterTest, TestPrepareCommitWithSpillAndMemory) {
    if (!GetParam()) {
        GTEST_SKIP() << "requires a real IOManager to create spill files";
    }
    ...
    std::shared_ptr<arrow::Array> memory_array =
        arrow::ipc::internal::json::ArrayFromJSON(value_type_, R"([
            ...
            .ValueOrDie();
    WriteBatch(memory_array, /*row_kinds=*/{}, merge_writer.get());

    ASSERT_OK_AND_ASSIGN(CommitIncrement commit_increment,
                         merge_writer->PrepareCommit(/*wait_compaction=*/false));
    ASSERT_EQ(0u, CountFiles(dir->Str() + "/tmp"));
    ASSERT_OK(merge_writer->Close());
...
TEST_P(MergeTreeWriterTest, TestIntermediateMergeKeepsSpillFileCountBounded) {
    if (!GetParam()) {
        GTEST_SKIP() << "requires a real IOManager to create spill files";
    }
