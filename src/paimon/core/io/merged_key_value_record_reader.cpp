...
Status MergedKeyValueRecordReader::Iterator::Init() {
    return LoadNextKeyValue();
}

Result<KeyValue> MergedKeyValueRecordReader::Iterator::Next() {
    if (!next_key_value_.has_value()) {
        return Status::Invalid("merged key value reader has no more rows");
    }
...
Result<std::unique_ptr<KeyValueRecordReader::Iterator>> MergedKeyValueRecordReader::NextBatch() {
    ...
    auto iterator = std::make_unique<MergedKeyValueRecordReader::Iterator>(this);
    PAIMON_RETURN_NOT_OK(iterator->Init());
    if (!iterator->HasNext()) {
        return std::unique_ptr<KeyValueRecordReader::Iterator>();
    }
    return iterator;
...
std::shared_ptr<Metrics> MergedKeyValueRecordReader::GetReaderMetrics() const {
    return reader_ == nullptr ? nullptr : reader_->GetReaderMetrics();
}
