...
struct CoreOptionsParser {
    ...
    static Status ParseBasicOptions(
        ...
        if (parser.ContainsKey(Options::TARGET_FILE_SIZE)) {
            ...
            PAIMON_RETURN_NOT_OK(
                parser.ParseMemorySize(Options::TARGET_FILE_SIZE, &target_file_size));
        ...
        PAIMON_RETURN_NOT_OK(parser.ParseMemorySize(Options::SOURCE_SPLIT_TARGET_SIZE,
                                                    &impl->source_split_target_size));
        ...
        PAIMON_RETURN_NOT_OK(parser.ParseFileSystem(fs_scheme_to_identifier_map,
                                                    specified_file_system, &impl->file_system));
        ...
        PAIMON_RETURN_NOT_OK(
            parser.ParseString(Options::DATA_FILE_PREFIX, &impl->data_file_prefix));
    ...
    static Status ParseFileFormatOptions(const ConfigParser& parser, CoreOptions::Impl* impl) {
        ...
        PAIMON_RETURN_NOT_OK(
            parser.ParseString(Options::FILE_COMPRESSION, &impl->file_compression));
        ...
        PAIMON_RETURN_NOT_OK(
            parser.ParseFileCompressionPerLevel(&impl->file_compression_per_level));
        return Status::OK();
    }

    // Parse manifest file configurations: format, compression, merge, and compaction thresholds.
    static Status ParseManifestOptions(const ConfigParser& parser, CoreOptions::Impl* impl) {
    ...
    static Status ParseExpireOptions(const ConfigParser& parser, CoreOptions::Impl* impl) {
    ...
    static Status ParseCommitOptions(const ConfigParser& parser, CoreOptions::Impl* impl) {
    ...
    static Status ParseMergeAndSequenceOptions(const ConfigParser& parser,
                                               CoreOptions::Impl* impl) {
        ...
        PAIMON_RETURN_NOT_OK(
            parser.ParseString(Options::FIELDS_DEFAULT_AGG_FUNC, &field_default_func));
    ...
    static Status ParseDeletionVectorOptions(const ConfigParser& parser, CoreOptions::Impl* impl) {
        ...
        PAIMON_RETURN_NOT_OK(parser.Parse<bool>(Options::DELETION_VECTOR_BITMAP64,
                                                &impl->deletion_vectors_bitmap64));
        return Status::OK();
    }

    // Parse scan, branch, and tag related configurations.
    static Status ParseScanAndBranchOptions(const ConfigParser& parser, CoreOptions::Impl* impl) {
        ...
        PAIMON_RETURN_NOT_OK(
            parser.ParseString(Options::SCAN_FALLBACK_BRANCH, &scan_fallback_branch));
    ...
    static Status ParseIndexOptions(const ConfigParser& parser, CoreOptions::Impl* impl) {
    ...
    static Status ParseCompactionOptions(const ConfigParser& parser, CoreOptions::Impl* impl) {
        ...
        if (!compaction_incremental_size_threshold_str.empty()) {
            PAIMON_ASSIGN_OR_RAISE(
                impl->compaction_incremental_size_threshold,
    ...
    static Status ParseLookupOptions(const ConfigParser& parser, CoreOptions::Impl* impl) {
        ...
        PAIMON_RETURN_NOT_OK(parser.Parse<bool>(Options::LOOKUP_REMOTE_FILE_ENABLED,
                                                &impl->lookup_remote_file_enabled));
        ...
        PAIMON_RETURN_NOT_OK(
            parser.ParseMemorySize(Options::CACHE_PAGE_SIZE, &impl->cache_page_size));
...
Result<CoreOptions> CoreOptions::FromMap(
    ...
    PAIMON_RETURN_NOT_OK(CoreOptionsParser::ParseBasicOptions(
        parser, impl.get(), specified_file_system, fs_scheme_to_identifier_map));
    PAIMON_RETURN_NOT_OK(CoreOptionsParser::ParseFileFormatOptions(parser, impl.get()));
    PAIMON_RETURN_NOT_OK(CoreOptionsParser::ParseManifestOptions(parser, impl.get()));
    PAIMON_RETURN_NOT_OK(CoreOptionsParser::ParseExpireOptions(parser, impl.get()));
    PAIMON_RETURN_NOT_OK(CoreOptionsParser::ParseCommitOptions(parser, impl.get()));
    PAIMON_RETURN_NOT_OK(CoreOptionsParser::ParseMergeAndSequenceOptions(parser, impl.get()));
    PAIMON_RETURN_NOT_OK(CoreOptionsParser::ParseDeletionVectorOptions(parser, impl.get()));
    PAIMON_RETURN_NOT_OK(CoreOptionsParser::ParseScanAndBranchOptions(parser, impl.get()));
    PAIMON_RETURN_NOT_OK(CoreOptionsParser::ParseIndexOptions(parser, impl.get()));
    PAIMON_RETURN_NOT_OK(CoreOptionsParser::ParseCompactionOptions(parser, impl.get()));
    PAIMON_RETURN_NOT_OK(CoreOptionsParser::ParseLookupOptions(parser, impl.get()));
