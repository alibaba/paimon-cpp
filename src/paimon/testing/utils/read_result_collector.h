...
class ReadResultCollector {
 public:
    ...
    static Result<std::shared_ptr<arrow::ChunkedArray>> CollectResult(
        BatchReader* batch_reader, int64_t max_data_processing_time_in_us = 0,
        int64_t max_read_batch = std::numeric_limits<int64_t>::max()) {
        arrow::ArrayVector result_array_vector;
        int64_t seed = DateTimeUtils::GetCurrentUTCTimeUs();
        std::srand(seed);
        int64_t read_batch_count = 0;
        while (read_batch_count < max_read_batch) {
