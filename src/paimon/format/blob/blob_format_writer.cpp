...
Status BlobFormatWriter::WriteBlob(std::string_view blob_data) {
    ...
    uint32_t read_len =
        std::min(static_cast<uint32_t>(file_length), static_cast<uint32_t>(tmp_buffer_->size()));
    while (read_len > 0) {
        ...
        read_len = std::min(static_cast<uint32_t>(file_length - total_read_length),
                            static_cast<uint32_t>(tmp_buffer_->size()));
    }
