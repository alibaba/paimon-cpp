...
TEST_P(AvroFileFormatTest, TestSimple) {
    ...
    ASSERT_TRUE(output_array->Equals(input_array)) << output_array->ToString() << "\n vs \n"
...
TEST_P(AvroFileFormatTest, TestComplexTypes) {
    ...
    auto input_array =
        arrow::ipc::internal::json::ArrayFromJSON(data_type, src_data_str).ValueOr(nullptr);
    ASSERT_TRUE(input_array);
    ::ArrowArray c_array;
    ASSERT_TRUE(arrow::ExportArray(*input_array, &c_array).ok());
    ASSERT_OK(writer->AddBatch(&c_array));
    ...
    ASSERT_TRUE(output_array->Equals(input_array)) << input_array->ToString() << std::endl
...
TEST_P(AvroFileFormatTest, TestReadRow) {
    ASSERT_OK_AND_ASSIGN(std::unique_ptr<FileFormat> file_format,
                         FileFormatFactory::Get("avro", {}));
    auto timezone = DateTimeUtils::GetLocalTimezoneName();
