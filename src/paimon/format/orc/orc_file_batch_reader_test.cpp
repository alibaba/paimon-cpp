...
TEST_P(OrcFileBatchReaderTest, TestNextBatchSimple) {
    ...
    for (auto batch_size : {1, 2, 3, 5, 8, 10}) {
        ...
        ASSERT_EQ(orc_batch_reader->GetPreviousBatchGlobalRowId(0).value(), -1);
        ASSERT_OK_AND_ASSIGN(auto result_array, paimon::test::ReadResultCollector::CollectResult(
                                                    orc_batch_reader.get()));
        ASSERT_EQ(orc_batch_reader->GetPreviousBatchGlobalRowId(0).value(), 8);
...
TEST_F(OrcFileBatchReaderTest, TestReadNoField) {
    ...
    ASSERT_EQ(orc_batch_reader->GetPreviousBatchGlobalRowId(0).value(), 0);
    // read 3 rows
    ASSERT_OK_AND_ASSIGN(auto batch2, orc_batch_reader->NextBatch());
    ASSERT_EQ(orc_batch_reader->GetPreviousBatchGlobalRowId(0).value(), 3);
    // read 2 rows
    ASSERT_OK_AND_ASSIGN(auto batch3, orc_batch_reader->NextBatch());
    ASSERT_EQ(orc_batch_reader->GetPreviousBatchGlobalRowId(0).value(), 6);
    // read rows with eof
